...
const Tile = (props) => {
	...
	// Determine the primary number to display
	if (row === 1) {
		return (
			<Paper
				elevation={0}
				sx={{ p: 2, width: "100%", color: "primary.light", bgcolor: "cardBackgroundLight.main", borderRadius: 1, height: "100%", ":hover": (redirectTo) && { boxShadow: 5, cursor: "pointer" } }}
	...
	return (
		<Paper
			square
			elevation={0}
			sx={{ p: 1.5, width: "100%", color: "primary.light", bgcolor: "cardBackgroundLight.main", borderRadius: 1, height: "100%", position: "relative", ":hover": (redirectTo) && { boxShadow: 5, cursor: "pointer" } }}
			onClick={() => (redirectTo) && navigate(redirectTo)}
		>
			<Box sx={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
				{(tooltip) && (
